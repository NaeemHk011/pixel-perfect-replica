...
export function Footer() {
  return (
    <footer className="bg-dark2 text-cream/80 pt-20 pb-8 relative z-10">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 grid grid-cols-1 md:grid-cols-4 gap-12">
        <div className="md:col-span-1">
          <div className="flex items-center gap-3">
            <img src={mark} alt="Mindova" className="h-10 w-10 object-contain" loading="lazy" decoding="async" />
          ...
          <p className="mt-3 text-sm text-cream/55 max-w-xs leading-relaxed">
          ...
          {/* HIPAA badge */}
          <div className="mt-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-gold2/30 bg-gold2/5 text-[11px] tracking-[0.2em] text-gold2 uppercase">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 mt-16 pt-8 border-t border-white/8 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-cream/40">
  ...
  return (
    <div>
      ...
      <ul className="space-y-3 text-sm">
        {links.map(([to, label]) => (
          <li key={label}>
            <Link to={to} className="text-cream/60 hover:text-gold3 transition-colors duration-200">
              {label}
            </Link>
          </li>
        ))}
