...
export function Navbar({ onDark = false }: { onDark?: boolean }) {
  ...
  return (
    <>
      ...
      >
        <div className="relative z-[1] max-w-7xl mx-auto px-5 md:px-8 h-20 lg:h-24 flex items-center justify-between gap-4">

          {/* ── Logo + Wordmark ── */}
          <Link to="/" className="flex flex-col items-start group shrink-0">
            <img
              src={logoFull}
              alt="Mindova Holdings"
              className="h-10 md:h-12 lg:h-14 object-contain"
            />
            {/* <span className="hidden md:block text-[9px] tracking-[0.22em] uppercase text-dark/45 font-semibold leading-none mt-0.5 pl-0.5">
              Mindova Holdings
            </span> */}
          </Link>
            ...
            <div className="flex items-center bg-dark text-cream rounded-full px-1.5 py-1.5 border border-white/5 shadow-[0_4px_16px_-8px_rgba(12,11,9,0.4)]">
              ...
              >
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs transition-all duration-200 flex items-center gap-1 whitespace-nowrap ${
          ...
          <div className="hidden xl:flex items-center gap-2 shrink-0">
            ...
            <Link
              to="/join-the-network"
              className={`inline-flex items-center gap-1.5 font-semibold px-5 py-2 rounded-full text-xs transition-all duration-200 hover:-translate-y-0.5 whitespace-nowrap uppercase tracking-wide ${
                transparent
                  ? "border border-white/50 text-white hover:border-gold2 hover:text-gold3"
                  : "border border-dark/25 text-dark hover:border-gold2 hover:text-gold3"
              }`}
            >
              Become a Provider
            </Link>
          ...
          <button
            ...
            className={`xl:hidden p-2 rounded-full transition-colors shrink-0 ${
      ...
      {open && (
        <div className="xl:hidden fixed inset-x-0 top-20 lg:top-24 bottom-0 bg-dark text-cream flex flex-col p-8 gap-5 z-[60] overflow-y-auto">
          ...
          <div className="relative">
            ...
            <div className="mt-3 ml-4 flex flex-col gap-1 border-l border-gold/20 pl-4">
              {serviceCategories.map(({ id, icon: Icon, label, color, bg }) => (
                <Link
                  key={id}
                  to={`/services/${id}`}
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            <Link
              to="/join-the-network"
              onClick={() => setOpen(false)}
              className="border border-cream/30 text-cream px-6 py-3.5 rounded-full text-center font-semibold text-sm hover:border-gold2 hover:text-gold3 transition-colors uppercase tracking-wide"
            >
              Become a Provider
            </Link>
          </div>
